...
public class Item {
	...
	public Item(String name, String IDname, String desc, int cost, int maxQ, int type, boolean unique, boolean useMenu) { //para items de verdad
		this.name = name;
		this.IDname = IDname;
		this.desc = desc;
		this.skillType = type;
	...
	public String Use(Entity a, Player player) {
		boolean use = true;
		...
		case "minSPot":
			if(a.sp<a.getSP()) {
			 ...
			 use = false;
			 errMsg = "La Stamina esta al maximo!";
			}
			break;
		case "minHPven":
			if(a.hp>0) {
